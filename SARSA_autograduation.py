...
class SARSA_autograduation():
	...
	def __init__(self):
		...
		self.N = 1
		self.tau = 100.0
		self.limit = 20.0 # only integer value
	...
	def one_function_graduation(self, funcname):
		...
		for t in range(3):
			...
			print self.goal
			# NEW REWARD
			self.sarsaad.test_analysis(self.goal)
			...
			for e in range(1):
				...
				try: sarsa = SARSA() # sarsa = SARSA(Nn x Nn (lattice), tau, visualization, data file)
				except: work = False
				...
				if(work):     # Script was working properly or not (within time limit or not) 
					...
					from SARSA_solution import SARSA_solution
					sarsas = SARSA_solution()
					sarsas.run(0)
					fastwayS, longwayS, overwayS, neverwayS = sarsaev.run_evaluation(0)
					#variS += vS/self.N
					print "Function:", fastwayF, longwayF, overwayF, neverwayF
					print "Solution:", fastwayS, longwayS, overwayS, neverwayS
					sumwayF = 1*fastwayF+(1-0.55+longwayF/max(overwayF,0.01))*longwayF+0*neverwayF
					sumwayS = 1*fastwayS+(1-0.55+longwayS/max(overwayS,0.01))*longwayS+0*neverwayS#len(np.nonzero(self.M)[1])#
					...
					fastwayS=1;   longwayS=1;   overwayS=1;   neverwayS=1;
